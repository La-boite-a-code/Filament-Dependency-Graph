...
2. **Middleware** has two lists. `middleware` is what is declared: the route definition plus the framework controller attributes (`#[Middleware]`, `#[Authorize]`, `#[WithoutMiddleware]`), excluded middleware removed, closures stored as `Closure`. `resolvedMiddleware` is what the router would run (`Router::resolveMiddleware()`, which resolves aliases, nested groups and exclusions however they are written, without instantiating anything), plus every alias and group name leading to one of those middleware; the middleware filter matches against it. Full-page Livewire components get their attributes read too. A controller's static `middleware()` method and application subclasses of the attributes are not read.
...
7. **Events without dispatcher** carry a dedicated `No dispatcher found` badge (Livewire components, commands and observers are not scanned, so the wording stays factual). `OrphanDetector` is unchanged: controller usage does not make a model non-orphan, so badges and orphan filters stay identical in the Filament and Laravel scopes.
...
11. **Laravel scope** excludes the HTTP node types explicitly, so it renders exactly what it rendered before. Models referenced only by routes or controllers join the model inventory like Livewire-referenced models do, but vendor ones only when `vendor_models.enabled` is on.
12. **Route cache**: closures restored from it have no file, so they are skipped with a `route_cache_closures_skipped` warning unless vendor routes are included.
13. **HTTP tree**: action-aware like the scope traversal, and model relations are not expanded in it (they belong to the other scopes).
14. **Policies** guarding several models become one node listing every model (`model_classes`) and every resolution source (`sources`), with one guard edge per model.
15. **Orphans in the HTTP scope**: hiding orphans there counts controller usage as a connection, so models reached from routes stay visible; the `Orphan` badge keeps describing the data model in every scope.
16. **Trait aliases** are resolved through `ReflectionClass::getTraitAliases()` to read the original method body.
