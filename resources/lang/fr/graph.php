<?php

declare(strict_types=1);

return [
    'navigation_label' => 'Graphe de dependances',
    'title' => 'Graphe de dependances',

    'toolbar' => [
        'scope' => 'Portee',
        'scope_filament' => 'Filament',
        'scope_laravel' => 'Laravel',
        'scope_http' => 'HTTP',
        'scope_views' => 'Vues',
        'view_graph' => 'Graphe',
        'view_tree' => 'Arbre',
        'view_table' => 'Table',
        'search_placeholder' => 'Rechercher modeles, routes, controleurs, composants Livewire, resources...',
        'export' => 'Exporter',
        'export_json' => 'Exporter en JSON',
        'export_mermaid' => 'Exporter en Mermaid',
        'reset' => 'Reinitialiser',
    ],

    'explorer' => [
        'title' => 'Explorateur',
        'description' => 'Filtrer les panels, les types de noeuds et les relations',
        'panels' => 'Panels',
        'node_types' => 'Types de noeuds',
        'relation_types' => 'Types de relations',
        'filters' => 'Filtres',
        'namespace' => 'Namespace contient',
        'ownership' => 'Provenance',
        'ownership_all' => 'Tous les modeles',
        'ownership_application' => 'Modeles applicatifs',
        'ownership_vendor' => 'Modeles vendor',
        'show_orphans' => 'Afficher les modeles orphelins',
        'only_orphans' => 'Orphelins uniquement',
        'only_cycles' => 'Dependances circulaires uniquement',
        'only_without_resource' => 'Modeles sans resource uniquement',
        'focus' => 'Focus',
        'focus_depth' => 'Profondeur',
        'focus_depth_unlimited' => 'Illimitee',
        'focus_direction' => 'Direction',
        'direction_incoming' => 'Entrantes',
        'direction_outgoing' => 'Sortantes',
        'direction_both' => 'Les deux',
        'exit_focus' => 'Quitter le focus',
        'middleware' => 'Middleware',
        'middleware_all' => 'Toutes les routes',
        'middleware_with' => 'Routes avec :name',
        'middleware_without' => 'Routes sans :name',
    ],

    'node_types' => [
        'panel' => 'Panels',
        'resource' => 'Resources',
        'livewire_component' => 'Composants Livewire',
        'model' => 'Modeles',
        'polymorphic_target' => 'Cibles polymorphes',
        'route' => 'Routes',
        'controller' => 'Controleurs',
        'form_request' => 'Form requests',
        'policy' => 'Policies',
        'event' => 'Evenements',
        'listener' => 'Listeners',
        'job' => 'Jobs',
        'mailable' => 'Mailables',
        'notification' => 'Notifications',
        'view' => 'Vues',
        'blade_component' => 'Composants Blade',
        'filament_component' => 'Composants Filament',
        'external_view' => 'Vues de packages',
        'dynamic_view' => 'Vues dynamiques',
    ],

    'workspace' => [
        'graph_title' => 'Graphe de dependances',
        'tree_title' => 'Arbre des dependances',
        'table_title' => 'Inventaire du projet',
        'empty' => 'Aucun noeud ne correspond aux filtres actuels.',
        'error' => 'Le graphe n\'a pas pu etre construit :',
        'stats' => ':nodes noeuds, :edges liens',
        'layout' => 'Disposition',
        'layout_hierarchical' => 'Hierarchique',
        'layout_force' => 'Force dirigee',
        'fit' => 'Ajuster a la vue',
        'controls' => 'Controles du graphe',
        'zoom_in' => 'Zoomer',
        'zoom_out' => 'Dezoomer',
        'expand' => 'Agrandir le graphe',
        'collapse' => 'Afficher l’explorateur',
    ],

    'inspector' => [
        'title' => 'Inspecteur',
        'close' => 'Fermer l\'inspecteur',
        'focus_node' => 'Focus sur ce noeud',
        'copy_value' => 'Copier la valeur',
        'copied' => 'Copie effectuee',
        'empty_section' => 'Rien a afficher.',
        'no_warnings' => 'Aucun avertissement detecte.',
        'types' => [
            'panel' => 'Panel Filament',
            'resource' => 'Resource Filament',
            'livewire_component' => 'Composant Livewire',
            'model' => 'Modele Eloquent',
            'polymorphic_target' => 'Cible polymorphe',
            'route' => 'Route',
            'controller' => 'Controleur',
            'form_request' => 'Form request',
            'policy' => 'Policy',
            'event' => 'Evenement',
            'listener' => 'Listener',
            'job' => 'Job',
            'mailable' => 'Mailable',
            'notification' => 'Notification',
            'view' => 'Vue Blade',
            'blade_component' => 'Composant Blade',
            'filament_component' => 'Composant Filament',
            'anonymous_component' => 'Composant anonyme',
            'external_view' => 'Vue de package',
            'dynamic_view' => 'Vue dynamique',
            'edge' => 'Dependance',
        ],
    ],

    'search' => [
        'no_results' => 'Aucun resultat.',
    ],

    'tree' => [
        'already_shown' => 'deja affiche',
        'undispatched_events' => 'Evenements sans declencheur detecte',
        'unreferenced_views' => 'Vues et composants sans reference detectee',
        'groups' => [
            'routes' => 'Routes et controleurs',
            'livewire' => 'Livewire',
            'filament' => 'Filament',
            'blade_components' => 'Composants Blade',
            'mail' => 'Mails',
        ],
        'empty' => 'Rien a afficher avec les filtres actuels.',
    ],

    'table' => [
        'dataset_label' => 'Categorie de l\'inventaire',
        'search_placeholder' => 'Rechercher dans cette categorie...',
        'items' => ':count elements',
        'empty' => 'Aucun element ne correspond aux filtres actuels.',
        'models' => 'Modeles',
        'model' => 'Modele',
        'namespace' => 'Namespace',
        'database_table' => 'Table',
        'resources_count' => 'Resources',
        'livewire_components_count' => 'Livewire',
        'outgoing' => 'Sortantes',
        'incoming' => 'Entrantes',
        'soft_deletes' => 'SoftDeletes',
        'status' => 'Statut',
        'relations' => 'Relations',
        'source' => 'Modele source',
        'method' => 'Methode',
        'type' => 'Type',
        'target' => 'Modele cible',
        'foreign_key' => 'Cle etrangere',
        'pivot' => 'Pivot',
        'nullable' => 'Nullable',
        'resources' => 'Resources',
        'resource' => 'Resource',
        'panels' => 'Panels',
        'navigation_group' => 'Groupe de navigation',
        'pages' => 'Pages',
        'relation_managers' => 'Relation managers',
        'livewire_components' => 'Composants Livewire',
        'component' => 'Composant',
        'alias' => 'Alias',
        'view' => 'Vue',
        'models_count' => 'Modeles',
        'properties' => 'Proprietes',
        'methods_count' => 'Methodes',
        'yes' => 'Oui',
        'no' => 'Non',
        'unknown' => 'Inconnu',
        'routes' => 'Routes',
        'route' => 'Route',
        'action' => 'Action',
        'middleware' => 'Middleware',
        'form_requests' => 'Form requests',
        'events' => 'Evenements',
        'event' => 'Evenement',
        'listeners' => 'Listeners',
        'queued_listeners' => 'Listeners en file',
        'dispatched_by' => 'Declenche par',
        'not_dispatched' => 'Aucun declencheur detecte',
        'dispatches' => 'Declenchements',
        'class' => 'Classe',
        'kind' => 'Nature',
        'queued' => 'En file',
        'views' => 'Vues',
        'components' => 'Composants',
        'externals' => 'Vues de packages',
        'view_name' => 'Vue',
        'used_by' => 'Utilisee par',
        'uses' => 'Utilise',
        'reference' => 'Reference',
        'package' => 'Package',
        'missing' => 'Manquante',
        'used_in_views' => 'Utilise dans des vues',
        'statuses' => [
            'complete' => 'Complet',
            'partial' => 'Partiel',
            'failed' => 'Echec',
        ],
    ],

    'view_kinds' => [
        'layout' => 'Layout',
        'page' => 'Page',
        'partial' => 'Partial',
        'component' => 'Composant',
        'livewire' => 'Livewire',
        'mail' => 'Mail',
    ],
];
